...
(async () => {
    ...
    while (!connectionSuccessful && !(retryTime > config.MAX_CONNECTION_RETRY)) {
        try {
            ...
            await require('./cypher/startup-setup')
        } catch (error) {
            console.log(
                `Retry: ${retryTime}, Connectivity verification failed. ${error}`
            )
            if (retryTime == config.MAX_CONNECTION_RETRY) {
                logger.warn(
                    `${Date.now()} : Unable to connect to db on startup with error: ${error} `
                )
                // TODO: switch to in-mem db (show confirmation or use config!)
            }
            retryTime += 1
            await new Promise((resolve) =>
                setTimeout(
                    resolve,
                    Math.min(5000, config.RECONNECT_TIME * retryTime)
                )
            )
        }
    }
