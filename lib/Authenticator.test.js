  ...
  describe("Testing Registeration", () => {
    ...
    describe("Testing Proctor Registeration", ()=> {
      ...
      test("Recieve token after Registering proctor by an administrator", async () => {
        ...
        let token = await authenticator.registerAdministrator(administrator);
        let proctor = new Proctor(administrator.administrator_id, "proctor1", "proctor123!", "proctor1", "proctor2");
        let proctorToken = await authenticator.registerProctor(
          proctor,
          token
        );
      ...
      test("Recive Error if username for Proctor is already resgistered, async style", async () => {
        ...
        let token = await authenticator.registerAdministrator(administrator);
        let proctor = new Proctor(null, "proctor1", "proctor123!", "proctor1", "proctor2");
        
        let prcoctorToken = await authenticator.registerProctor(
          proctor,
          token
        );
        try {
      ...
      afterAll(async () => {
        await pool.query(adminQueries.truncateAdministrators);
        await pool.query(proctorQueries.truncateExaminees);
      });
