...
class Authenticator {
    ...
    async registerAdministrator(administrator) {
        try{
            let res = await this.pool.query(adminQueries.insert.usernamePasswordNameReturning, [administrator.username, await Authenticator.encryptPayload(administrator.password), administrator.first_name, administrator.last_name]);
    ...
    async registerExaminee(examinee, administratorToken) {
        try {
            ...
            let res = await this.pool.query(examineeQueries.insert.usernamePasswordNameReturning, [decoded.administrator_id, examinee.username, await Authenticator.encryptPayload(examinee.password), examinee.first_name, examinee.last_name]);
            return Authenticator.createToken(res.rows[0]);
        } catch(err) {
            if (err.code == errors.DUPLICATE_KEY_VALUE) throw new AuthenticationError('Username already exists');
            throw new AuthenticationError(err);
